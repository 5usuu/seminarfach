...
/* Baut das Markup für das Produktbild: echtes Foto, falls vorhanden, sonst
   automatisch das Emoji als Fallback – so bricht nichts, solange noch
   nicht für jedes Produkt ein Foto hinterlegt ist.
   Unterstützt ZWEI Arten von Fotos:
   - p.photos = { schwarz: '...', weiss: '...', ... } -> ein eigenes Foto
     je Farbe (z.B. echte Produktfotos in verschiedenen Farbvarianten).
     "colorId" wählt aus, welches Foto gezeigt wird (Standard: erste
     verfügbare Farbe von p.colors).
   - p.photo = einzelner Pfad, falls es nur EIN Foto gibt (z.B. das Logo
     als Platzhalter, unabhängig von der Farbe).
   WICHTIG: Der Emoji-Fallback greift auch, wenn ein Foto-Pfad zwar
...
function resolveProductPhoto(p, colorId) {
  if (p.photos) {
    const cid = colorId || (p.colors && p.colors[0]);
    if (cid && p.photos[cid]) return p.photos[cid];
    // Fallback: irgendein vorhandenes Foto aus "photos", falls die
    // gewünschte Farbe (noch) kein eigenes Foto hat.
    const firstAvailable = Object.values(p.photos).find(Boolean);
    if (firstAvailable) return firstAvailable;
  }
  return p.photo || '';
}

function productImageHTML(p, extraAttrs, colorId) {
  extraAttrs = extraAttrs || '';
  const fallbackEmoji = p.emoji || p.image || '📦';
  const photoSrc = resolveProductPhoto(p, colorId);
  if (photoSrc) {
    return '<img src="' + photoSrc + '" alt="' + p.name + '" loading="lazy" ' + extraAttrs +
...
/* Wird vom onerror-Handler oben aufgerufen, falls ein Produktfoto nicht
   ...
   durch dieselbe Emoji-Darstellung, die auch ohne Foto verwendet würde. */
...
const PRODUCTS = [
  ...
  {
    id: 'rueckwand', name: 'GG Hildburghausen Rucksack', emoji: '🎒',
    badge: 'NEU', badgeClass: 'new',
  ...
  },
  {
    ...
    stockLeft: 35, viewers: 4, timerStart: '12:41:09', colors: ALL_COLOR_IDS,
